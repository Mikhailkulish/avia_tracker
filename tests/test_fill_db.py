...
from src.fill_db import (DB_HOST, DB_NAME, DB_PASSWORD, DB_PORT, DB_USER, MAX_AIRCRAFT_PER_COUNTRY, MIN_COUNTRIES,
                         REQUEST_DELAY, add_country, clear_data, create_db, fill_all_countries, fill_country,
                         get_connection, get_countries_from_user, get_country_id, get_existing_aircraft_in_country,
                         save_aircraft, save_track)


# Тестирование функции get_connection
def test_successful_connection(mock_successful_connection: Mock, mock_logger: Mock) -> None:
...
def test_successful_connection_custom_dbname(mock_successful_connection: Mock, mock_logger: Mock) -> None:
    """Тест успешного подключения с пользовательским именем БД"""
    custom_dbname = "test_db"

    _ = get_connection(custom_dbname)

    mock_successful_connection.connect.assert_called_once_with(
        dbname=custom_dbname, user=DB_USER, password=DB_PASSWORD, host=DB_HOST, port=DB_PORT
    )
    mock_logger.debug.assert_called_once_with(f"Подключение к БД {custom_dbname} установлено")


def test_connection_failure(mock_failed_connection: Mock, mock_logger: Mock) -> None:
...
def test_connection_failure_custom_dbname(mock_failed_connection: Mock, mock_logger: Mock) -> None:
...
def test_create_db_database_not_exists(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    mock_get_connection.assert_has_calls([call("postgres"), call()])

    # Проверяем создание БД
    mock_cur_postgres.execute.assert_any_call("SELECT 1 FROM pg_database WHERE datname = %s", (DB_NAME,))
...
def test_create_db_database_exists(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    mock_cur_postgres.execute.assert_any_call("SELECT 1 FROM pg_database WHERE datname = %s", (DB_NAME,))
    # Проверяем, что CREATE DATABASE НЕ вызывался
    create_calls = [call for call in mock_cur_postgres.execute.call_args_list if "CREATE DATABASE" in str(call)]
...
def test_create_db_connection_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    mock_logger.error.assert_called_once_with("Ошибка при создании базы данных: Connection refused")


def test_create_db_creation_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    mock_logger.error.assert_called_once_with("Ошибка при создании базы данных: Database creation failed")


def test_create_db_table_creation_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    mock_logger.error.assert_called_once_with("Ошибка при создании базы данных: Table creation failed")


# Тестирование функции clear_data
def test_clear_data_success(mock_get_connection: Mock, mock_logger: Mock, capsys) -> None:
...
def test_clear_data_error_handling(mock_get_connection: Mock, mock_logger: Mock, capsys) -> None:
...
def test_add_country_success(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_add_country_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    mock_logger.error.assert_called_once_with("Ошибка добавления страны 'Russia': Database error")
...
def test_save_aircraft_success(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_save_aircraft_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    mock_logger.error.assert_called_once_with("Ошибка сохранения самолета ABC123: Database error")
    mock_conn.commit.assert_not_called()
    # close НЕ вызывается при ошибке
    mock_conn.close.assert_not_called()
...
def test_save_track_success(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_save_track_duplicate(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_save_track_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    mock_logger.error.assert_called_once_with("Ошибка сохранения трека для ABC123: Database error")
    mock_conn.commit.assert_not_called()
    mock_conn.close.assert_not_called()


def test_save_track_connection_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    """Тест ошибки подключения"""
    mock_get_connection.side_effect = psycopg2.OperationalError("Connection refused")

    save_track(1, 1, "ABC123", 10000.5, 500.25, 55.7558, 37.6173)

    mock_logger.error.assert_called_once_with("Ошибка сохранения трека для ABC123: Connection refused")


# Тестирование функции get_country_id
def test_get_country_id_success(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_get_country_id_not_found(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_get_country_id_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    assert result is None
    mock_logger.error.assert_called_once_with("Ошибка получения ID для 'Russia': Database error")


# Тестирование функции get_existing_aircraft_in_country
def test_get_existing_aircraft_in_country_success(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_get_existing_aircraft_in_country_empty(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_get_existing_aircraft_in_country_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    assert result == set()
    mock_logger.error.assert_called_once_with("Ошибка получения существующих самолетов: Database error")


# Тестирование функции fill_country
def test_fill_country_success(mock_get_connection: Mock, mock_logger: Mock) -> None:
    """Тест успешного заполнения страны"""
    with (
        patch("src.fill_db.add_country"),
        ...
        mock_api = Mock()
        mock_api.aeroplanes = {
            "states": [["ABC123", "FL123", "Russia", None, None, 37.6173, 55.7558, 10000.5, None, 500.25]]
        }
...
def test_fill_country_no_data(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_fill_country_limit_reached(mock_get_connection: Mock, mock_logger: Mock) -> None:
...
def test_fill_country_critical_error(mock_get_connection: Mock, mock_logger: Mock) -> None:
    ...
    with patch("src.fill_db.add_country") as mock_add_country:
        ...
        assert result == 0
        mock_logger.error.assert_called_once_with("Критическая ошибка при заполнении Russia: Critical error")


# Тестирование функции get_countries_from_user
def test_get_countries_from_user_success(mock_logger: Mock) -> None:
...
def test_get_countries_from_user_with_extra(mock_logger: Mock) -> None:
...
def test_get_countries_from_user_stop(mock_logger: Mock) -> None:
...
def test_get_countries_from_user_duplicate(mock_logger: Mock) -> None:
...
def test_get_countries_from_user_not_enough(mock_logger: Mock) -> None:
...
def test_fill_all_countries_success(mock_logger: Mock) -> None:
    """Тест успешного заполнения всех стран"""
    countries = ["Russia", "USA"]

    with (
        patch("src.fill_db.create_db") as mock_create_db,
...
def test_fill_all_countries_empty(mock_logger: Mock) -> None:
...
def test_fill_all_countries_error(mock_logger: Mock) -> None:
    """Тест критической ошибки"""
    countries = ["Russia"]
    ...
    with patch("src.fill_db.create_db") as mock_create_db:
        ...
        mock_logger.error.assert_called_once_with("Критическая ошибка при сборе данных: DB error")
